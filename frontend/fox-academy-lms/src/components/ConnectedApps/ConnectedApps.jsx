...
const styles = {
  ...
  appsList: { display: 'flex', flexDirection: 'column', gap: 0 },
  appRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid #e5e7eb' },
  appRowLast: { borderBottom: 'none', paddingBottom: 0 },
  appInfo: { display: 'flex', alignItems: 'center', gap: '12px' },
  appIcon: { width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f3f4f6', borderRadius: '4px', border: '1px solid #e5e7eb', flexShrink: 0 },
  appName: { fontSize: '14px', fontWeight: '500', color: '#111827' },
  appConnectedGroup: { display: 'flex', alignItems: 'center', gap: '8px' },
  appStatus: { fontSize: '13px', fontWeight: '600' },
  appStatusConnected: { color: '#10b981' },
  appDisconnect: { width: '20px', height: '20px', borderRadius: '9999px', border: '1px solid #d1d5db', backgroundColor: 'transparent', color: '#9ca3af', fontSize: '14px', lineHeight: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s, color 0.15s', cursor: 'pointer' },
  appDisconnectHover: { backgroundColor: '#fee2e2', color: '#ef4444', borderColor: '#fecaca' },
  appConnectBtn: { fontSize: '13px', fontWeight: '600', color: '#d97706', backgroundColor: 'transparent', border: 'none', padding: 0, textDecoration: 'underline', textDecorationOffset: '2px', transition: 'opacity 0.15s', cursor: 'pointer' },
}
  ...
  return (
    <section style={styles.card}>
      <div style={styles.cardHeader}>
        <h2 style={styles.cardTitle}>Connected Apps</h2>
      </div>

      <div style={styles.appsList}>
        {apps.map((app, idx) => (
          <div key={app.id} style={{...styles.appRow, ...(idx === apps.length - 1 ? styles.appRowLast : {})}}>
            <div style={styles.appInfo}>
              <span style={styles.appIcon}>{app.icon}</span>
              <span style={styles.appName}>{app.name}</span>
            </div>
            {app.connected ? (
              <div style={styles.appConnectedGroup}>
                <span style={{...styles.appStatus, ...styles.appStatusConnected}}>Connected</span>
                <button
                  style={styles.appDisconnect}
                  onMouseEnter={(e) => Object.assign(e.target.style, styles.appDisconnectHover)}
                  onMouseLeave={(e) => Object.assign(e.target.style, {backgroundColor: 'transparent', color: '#9ca3af', borderColor: '#d1d5db'})}
