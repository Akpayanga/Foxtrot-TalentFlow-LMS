...
const { sendVerificationEmail, sendPasswordResetEmail } = require("../utilities/email.util");
...
exports.googleUserInstructorLogin = async (req, res, next) => {
  try {
    ...
    const redirectUrl = `${frontendUrl}/login?oauth=1&token=${encodeURIComponent(token)}&email=${encodeURIComponent(user.email || "")}&firstName=${encodeURIComponent(user.firstName || "")}&lastName=${encodeURIComponent(user.lastName || "")}&role=${encodeURIComponent(user.role || "student")}&course=${encodeURIComponent(user.course || "")}`;
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!emailSent) {
      throw new ApiError(500, "Unable to send password reset email right now");
    }

    await recordAudit({ userId: user._id, action: "FORGOT_PASSWORD", details: "Reset token generated", req });

    return success(res, null, "Password reset link sent successfully");
  } catch (err) {
    next(err);
  }
};

exports.resendForgotPassword = async (req, res, next) => {
  try {
    const { email } = req.body;
    const user = await User.findOne({ email, provider: "local" }).notDeleted();
    if (!user) throw new ApiError(404, "User not found");
    ...
    if (!emailSent) {
      throw new ApiError(500, "Unable to resend password reset email right now");
    }

    await recordAudit({ userId: user._id, action: "FORGOT_PASSWORD_RESEND", details: "Reset link resent", req });
    return success(res, null, "Password reset link sent again. Please check your inbox.");
  } catch (err) {
    next(err);
  }
};
